...
def optimize_z_for_min_false_negatives(sub, z_range):
    best_z = None
    min_fn = float('inf')
    best_stats = None
    concentration_col = "Concentration\n(ng)"
    ...
    for z in z_range:
        ...
        tn, fp, fn, tp = robust_confusion_matrix(y_true, preds)
        total = tp + tn + fp + fn
...
def save_analysis_excel(input_csv, output_excel, z_input=5.5):
    with pd.ExcelWriter(output_excel, engine='xlsxwriter',
        ...
        summary_rows = []
        N = 100
        z_min = max(0, z_input - 2)
        z_max = z_input + 2
        z_list = np.linspace(z_min, z_max, N).tolist()
        for compound in compounds:
            sub = df[df['compound_id'] == compound]
            compound_title = sub['compound_title'].iloc[0]
            cv = sub['critical_value'].iloc[0]
            y_true = (sub["Concentration\n(ng)"] != 0).astype(int)
            y_score = sub["Intensity\n(cps)"]
            # calculate optimal z using only this range
            best_z, min_fn, stats = optimize_z_for_min_false_negatives(sub, z_list)
            tp, tn, fp, fn, acc, recall, specificity, precision, f1 = stats
            # ensure best_z is included in z_list for summary/roc
            full_z_list = sorted(set(z_list + [best_z]))
            if len(y_true) == 0 or len(np.unique(y_true)) < 2:
        ...
        for compound in compounds:
            ...
            cv = sub['critical_value'].iloc[0]
            # Get best_z again so it's on ROC
            best_z, min_fn, stats = optimize_z_for_min_false_negatives(sub, z_list)
            full_z_list = sorted(set(z_list + [best_z]))
            if len(set(y_true)) < 2:
            ...
            try:
                ...
                auc_val = None
            roc_df = pd.DataFrame({'FPR': fpr, 'TPR': tpr, 'Threshold': threshs, 'z': full_z_list})
        ...
        chart.set_title({'name': 'All Compounds ROC Curves (Discrete z sweep)'})
